...
mechanical_velocity_om_m = linspace(0, 2*P.wmb, 500);  % mechanical rad/s
electrical_angular_velocity = (P.p/2).*mechanical_velocity_om_m;             % electrical rad/s
...
for k = 1:numel(field_current_cases)
    ...
    % compute voltage requirements (vectorized over speed)
    [V, Vq, Vd, V0] = sm_required_voltage_park(mechanical_velocity_om_m, If, Iq, Id, P);
