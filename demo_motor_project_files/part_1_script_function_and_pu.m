...
save_graphs = false; % set to true to save graphs to outputs directory
...
fprintf('  Rf   = %.2f ohm\n', P.Rf)
fprintf('  p    = %d poles\n', P.p)
fprintf('  wb   = %.1f rad/s  (%.1f hz)\n', P.wb, P.f_e_at_elect_base)
fprintf('  wmb  = %.1f rad/s  (%.1f rpm)\n', P.wmb, P.n_sync_rpm_at_elect_base)
fprintf('\ninverter params:\n')
fprintf('  Vdc     = %.0f V\n', P.Vdc)
...
mechanical_velocity_om_m = linspace(0, 3*P.wmb, 500);  % mechanical rad/s
...
for k = 1:numel(field_current_cases)
    ...
    % all-torque current (Id=0)
    Iq = P.I0_max;
    Id = 0;

    % compute voltage requirements in per-unit, convert back to SI
    [V_pu, Vq_pu, Vd_pu, V0_pu] = sm_required_voltage_park_pu( ...
        mechanical_velocity_om_m, If, Iq, Id, P, B);
    ...
    idx = find(V0_pu >= V0_lim_pu, 1, 'first'); % find the index of the first element exceeding limit
    if ~isempty(idx) % makes sure we found something
        cross_idx(k)   = idx; % store index into array the size of the current cases
        cross_speed(k) = mechanical_velocity_om_m(idx); % store corresponding speed
    end
end
...
for k = 1:numel(field_current_cases)
    ...
    if ~isnan(cross_idx(k)) % then this is marker stuff for graphing
...
if save_graphs && isvalid(fig)
    saveas(fig, fullfile(outputs_directory, 'part_1_plot_1_inverter_V0_vs_speed.png'));
end
