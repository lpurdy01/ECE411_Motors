...
%% ================= HW2 g-ii: Closed-loop speed step (K = 100%) =================
% Assumes variables R, L, K, Jm, B, stim_step already set as in your block above.
% Inner current-loop PI (with back-EMF decoupling) per g-i:
Kpa = 2*pi*300*L;                 % [V/A]
Kia = 2*pi*10*Kpa;                % [V/(A*s)]

% Outer speed-loop PI (outputs i_a^*)
Kpw = Jm*2*(2*pi*40);             % [A/(rad/s)]
Kiw = Jm*(2*pi*40)^2;             % [A/(rad/s*s)]

% Sim setup
mdl = 'g_closed_loop_speed_simulation';
stopTime = '0.1';
stim_step = 100;                  % w* step at t=0 [rad/s]
Tl_initial = 0; Tl_final = 0.5; Tl_step_t = 0.05;   % no load torque for g-ii

in = Simulink.SimulationInput(mdl);
in = in.setVariable('K',   K);    % rated flux only for g-ii
in = in.setVariable('Kpa', Kpa);
in = in.setVariable('Kia', Kia);
in = in.setVariable('Kpw', Kpw);
in = in.setVariable('Kiw', Kiw);
in = in.setVariable('stim_step',  stim_step);
in = in.setVariable('Tl_initial', Tl_initial);
in = in.setVariable('Tl_final',   Tl_final);
in = in.setVariable('Tl_step_t',  Tl_step_t);
in = in.setModelParameter('StopTime', stopTime);

Ssp = sim(in);

% Pull logged signals (adjust names here if your bus fields differ)
t     = Ssp.tout;
w_cmd = Ssp.w_cmd.Data;   % expected: timeseries out.w_cmd
w_meas= Ssp.w.Data;       % expected: timeseries out.w

% -------- Plot: speed command vs measured speed --------
figure('Color','w','Name','g-ii: Closed-loop speed step');
hold on; grid on; box on;
plot(t, w_cmd,  'k--', 'LineWidth',1.6, 'DisplayName','\omega^* (100 rad/s)');
plot(t, w_meas, 'r',   'LineWidth',2.1, 'DisplayName','\omega (measured)');
xlabel('Time  t  [s]'); ylabel('\omega  [rad/s]');
title(sprintf('g)vi)Closed-loop speed response', K, Kpw, Kiw));
legend('Location','best');
xlim([0 str2double(stopTime)]);

% -------- Quick metrics for your write-up --------
try
    info = stepinfo(w_meas, t, stim_step, 'RiseTimeLimits',[0.1 0.9], 'SettlingTimeThreshold',0.02);
    OS = 100*(max(w_meas)-stim_step)/max(1e-9,abs(stim_step));
    fprintf('\n[g-ii] Speed-loop metrics:\n');
    fprintf('  Rise time (10–90%%): %.4f s\n', info.RiseTime);
    fprintf('  Settling time (±2%%): %.4f s\n', info.SettlingTime);
    fprintf('  Overshoot: %.2f %%\n\n', OS);
catch
