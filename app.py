                        ...
                        [
                            ...
                            html.Div(
                                [
                                    ...
                                    html.Pre(id="frames-summary", style={"whiteSpace": "pre-wrap"}),
                                ],
                                className="tab-plots",
                            ),
                        ],
                        ...
                        [
                            ...
                            html.Div(
                                [
                                    ...
                                    html.Pre(id="round-summary", style={"whiteSpace": "pre-wrap"}),
                                ],
                                className="tab-plots",
                            ),
                        ],
                        ...
                        [
                            ...
                            html.Div(
                                [
                                    ...
                                    html.Pre(id="fw-summary", style={"whiteSpace": "pre-wrap"}),
                                ],
                                className="tab-plots",
                            ),
                        ],
                        ...
                        [
                            ...
                            html.Div(
                                [
                                    ...
                                    html.Pre(id="salient-summary", style={"whiteSpace": "pre-wrap"}),
                                ],
                                className="tab-plots",
                            ),
                        ],
...
def update_frames_tab(amplitude, frequency, phase, harmonic_order, harmonic_ratio, sync_value, theta_deg):
    ...
    summary = (
        f"Balanced set? {'Yes' if np.allclose(v0, 0, atol=1e-3) else 'No'}\n"
        f"Space-vector magnitude (rms) ≈ {np.sqrt(np.mean(np.square(np.abs(space_vector)))):.3f} pu\n"
        f"Mean q-axis value = {np.mean(vq):.3f}, mean d-axis value = {np.mean(vd):.3f}"
    )
...
def update_round_tab(rs, ls, v0, e0, w_e, delta_deg):
    ...
    summary = (
        f"Iq = {op['iq']:.3f} pu, Id = {op['id']:.3f} pu, |I| = {op['i_mag']:.3f} pu\n"
        f"Electrical power = {op['power']:.3f} pu\n"
        f"Torque = {op['torque']:.3f} pu\n"
        f"Power factor = {op['pf']:.3f} (angle {math.degrees(op['pf_angle']):.1f}°)"
    )
...
def update_field_weakening(v0_max, i0_max, ls, rs, selected_speed):
    ...
    summary = (
        f"Selected ωₑ = {selected_speed:.2f} pu\n"
        f"Required flux linkage Λ₀ ≈ {flux_selected:.3f} pu\n"
        f"Torque limit ≈ {torque_selected:.3f} pu, power limit ≈ {power_selected:.3f} pu"
    )
...
def update_salient_tab(ld, lq, e0, v0, w_e):
    ...
    summary = (
        f"Field torque max ≈ {field_max:.3f} pu\n"
        f"Reluctance torque max ≈ {rel_max:.3f} pu\n"
        f"Total torque max ≈ {total_max:.3f} pu\n"
        f"Saliency ratio L_d/L_q = {saliency:.3f}"
    )
